...
class bst {
  ...
  private: 
    struct bst_node {
      ...
      int numLeft;
	    int numRight;
      /*bst_node ( const T & _val = T{}, bst_node * l = nullptr, bst_node *r = nullptr, const T & _numL = T{}, const T & _numR = T{})
        : val { _val },  left { l }, right {r} , numLeft{_numL} , numRight{_numR}
      { }*/
 ...
 */
  ...
  public:
    ...
    void addInOrder(bst_node *r, int a[], int *i){
      //if node is null
      if(r==NULL) 
        return;
      //recursively add the left child
      addInOrder(r->left, a, i);
      //assign i as the node's value
      a[*i] = r->val;
    ...
    std::vector<T> * to_vector() {
      ...
      int i=0;
      //fill the vector with values from the tree
      //addInOrder(r,tree,&i)
      //returned filled-in vector
      return tree;
    }
    ...
    int ith_smallest(bst_node *r, int a){
      ...
      if((r->left)->val < a)
        return ith_smallest(r->right, a - (r->left)->val - 1);
    ...
    bool get_ith(int i, T &x) {
      if(i < 1 || i > size()){
        std::cout << "ERROR: i must range between 1 and the number of elements in the tree. But the input is "<< i <<" \n";
        return false;
        }
    ...
    static void _postorder(bst_node *r, int margin){
      if(r==nullptr){
        ...
        std::cout << "[ " << r->val << " ]\n";
          ...
          //std::cout << " Value:  "<< r->val << "\n";
          //std::cout << " SizeLeft:  "<< r->numLeft << ", SizeRight:  "<< r->numRight << "\n";
        }
